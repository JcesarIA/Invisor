...
public  void additem(){
    ...
    if (itembarcodeValue.isEmpty()) {
        itembarcode.setError("Está vazio");
    ...
    if(!TextUtils.isEmpty(itemnameValue)&&!TextUtils.isEmpty(itemcategoryValue)&&!TextUtils.isEmpty(itempriceValue)){
        ...
        itemname.setText("nome");
        itembarcode.setText("codigo de barras");
        itemprice.setText("preço");
        itembarcode.setText("codigo de barras");
        Toast.makeText(additemActivity.this,itemnameValue+" Adicionado",Toast.LENGTH_SHORT).show();
    }
    else {
    ...
    private void Logout()
    {
        ...
        Toast.makeText(additemActivity.this,"LOGOUT Bem Sucedido", Toast.LENGTH_SHORT).show();
